...
<!--the Clark reference looks like it has not rendered correctly in the vignette PDF.-->
...
<!--The white 2007 reference also appears not to render correctly in the PDF-->
...
Numerous variables were measured and are elaborate in the package help file for the dataset 
...
\end{array}
\right) \approx D
<!--\left[--> <!--Jon is just commenting these out for now, but the brackets appear unnecessary-->
\left(
\begin{array}{c}
...
\end{array}
\right)
<!--\right],--> <!--other side of the brackets-->
\end{equation}
...
We simulated three environmetnal drivers: an "operating" driver that strongly influences population dynamics (i.e., $q_{i1}^{(1)}=1$, $q_{il}^{(1)}=0$ for $l>1$) and is spatially structured; a "latent" driver that is also spatially structured but does not influence population dynamics (i.e., $q_{il}^{(2)}=0$ for all $l$); 
and a third driver 
